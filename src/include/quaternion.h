...
#ifndef RAYTRACER_QUATERNION_H_
#define RAYTRACER_QUATERNION_H_
...
public:
  ...
  Quaternion<_Tp> operator*(const Quaternion<_Tp> &q) const;

  Vector3d<_Tp> operator*(const Vector3d<_Tp> &v) const;
  ...
  //operator<<
  // -parameters : std::ostream o, quaternion q
  // -return values :
  ...
  Quaternion<_Tp> scale(_Tp s) const;
...
// -when called on a quaternion q2, multiplies q2 *q1  and returns the product in a new quaternion
template <class _Tp>
Quaternion<_Tp> Quaternion<_Tp>::operator*(const Quaternion<_Tp> &q) const
{
  return Quaternion(
...
// -when called on a quaternion object q, returns the magnitude q
template <class _Tp>
_Tp Quaternion<_Tp>::magnitude() const
{
  return sqrt(norm());
}
...
// -returns the original quaternion with each part, w,x,y,z, multiplied by some scalar s
template <class _Tp>
Quaternion<_Tp> Quaternion<_Tp>::scale(_Tp s) const
{
...
// -when called on a quaternion object q, returns the inverse of q
template <class _Tp>
Quaternion<_Tp> Quaternion<_Tp>::inverse() const
{
...
// -when called on a quaternion object q, returns the conjugate of q
template <class _Tp>
Quaternion<_Tp> Quaternion<_Tp>::conjugate() const
{
...
// -when called on quaterion q, takes q and returns the unit quaternion of q
template <class _Tp>
Quaternion<_Tp> Quaternion<_Tp>::UnitQuaternion() const
{
...
template <class _Tp>
void Quaternion<_Tp>::QuatRotation(_Tp v[3]) const
{
...
template <class _Tp>
inline Vector3d<_Tp> Quaternion<_Tp>::operator*(const Vector3d<_Tp> &v) const
{
  Vector3d<_Tp> ans;
  Quaternion<_Tp> qv(0, v[0], v[1], v[2]);
  Quaternion<_Tp> qm = (*this) * qv * (*this).inverse();
