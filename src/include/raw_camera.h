...
class RawCamera : public CameraBase
{
...
public:
  ...
  RawCamera(const vec3 &lower_left_corner_, const vec3 &horizontal_, const vec3 &vertical_, const vec3 &origin_)
      : lower_left_corner(lower_left_corner_), horizontal(horizontal_), vertical(vertical_), origin(origin_)
  {
  }

  RawCamera(const double &vfov, const double &aspect)
  {
  ...
  RawCamera(const vec3 &lookfrom, const vec3 &lookat, const vec3 &vup, const double &vfov, const double &aspect)
  {
    vec3 u, v, w;
    double theta = vfov * RadUnit;
    double half_height = std::tan(theta * 0.5);
