#ifndef RAYTRACER_RENDERER_H_
#define RAYTRACER_RENDERER_H_
...
public:
  ...
  std::vector<HitablePtr> random_scene()
  {
    ...
    for (int a = -6; a < 6; a++)
    {
      for (int b = -6; b < 6; b++)
      {
        float choose_mat = sampler01_.sample();
        vec3 center(a + 0.9 * sampler01_.sample(), 0.2, b + 0.9 * sampler01_.sample());
        if ((center - vec3(4, 0.2, 0)).length() > 0.9)
        {
            ...
            list.emplace_back(std::make_shared<Sphere>(center, 0.2,
                                                       std::make_shared<Lambertian>(
                                                           std::make_shared<ConstantTexture>(
                                                               vec3(sampler01_.sample() * sampler01_.sample(),
                                                                    sampler01_.sample() * sampler01_.sample(),
                                                                    sampler01_.sample() * sampler01_.sample())))));

            // list.emplace_back(std::make_shared<MovingSphere>(center, center + vec3(0, 0.5 * sampler01_.sample(), 0),
            //                                                  0, 1, 0.2,
          ...
          {
            list.emplace_back(std::make_shared<Sphere>(center, 0.2, std::make_shared<Metal>(vec3(0.5 * (1 + sampler01_.sample()), 0.5 * (1 + sampler01_.sample()), 0.5 * (1 + sampler01_.sample())), 0.5 * sampler01_.sample())));
          }
          else
          {
  ...
  std::vector<HitablePtr> next_week_final()
  {
    ...
    for (int i = 0; i < nb; i++)
    {
      for (int j = 0; j < nb; j++)
      {
        ...
        float y0 = 0;
        float x1 = x0 + w;
        float y1 = 100 * (sampler01_.sample() + 0.01);
        float z1 = z0 + w;
    ...
    for (int j = 0; j < ns; j++)
    {
      boxlist2[j] = std::make_shared<Sphere>(vec3(165 * sampler01_.sample(), 165 * sampler01_.sample(), 165 * sampler01_.sample()), 10, white);
    }
    ...
    for (int j = ny - 1; j >= 0; j--)
    {
      for (int i = 0; i < nx; i++)
      {
        ...
        for (int s = 0; s < sampling_count; s++)
        {
          ...
          col += color(camera->get_ray(u, v), hitables, light_shape, 0);
        }
