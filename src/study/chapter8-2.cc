...
int main()
{
  ...
  MaterialPtr metal = std::make_shared<Metal>(0.9 * OneAll);
  ...
  list.emplace_back(std::make_shared<Sphere>(vec3(0, 0, -1), 0.5, metal));
  list.emplace_back(std::make_shared<Sphere>(vec3(1, 0, -1), 0.3, lambertian));
  list.emplace_back(std::make_shared<Sphere>(vec3(-1, 0, -1), 0.3, lambertian2));
